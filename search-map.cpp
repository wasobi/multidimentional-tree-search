...
int main(int argc, char **args) {
    ...
    if (read_args(argc, args, &params)) {
        ...
        if (myfile.is_open()) {
            ...
            myfile.close();
            toc = std::clock();
            std::cerr << "\tdone! " << ktree.getSize() << " records inserted in " << (double)(toc-tic)/CLOCKS_PER_SEC << " secs\n";
            
            // make (only) one range query to the database
            std::cerr << "Fetching results ... ";
            tic = std::clock();
            count = ktree.printNeighbors(params.latitude, params.longitude, params.radius, params.filter);
            toc = std::clock();
